...
int main(){
    ...
    vector<string> Instructions = {
        ...
        "00000010000000000000000101101111",
        "00000000000100001001100001100011",
        "00000000001100010000000010110011",
        "01000000001100010000000010110011",
        "00000000001100010000000010110011",
        "01000000001100010000000010110011",
    ...
    while (!stop){
        cout << "Clock Cycle " << cycleCnt++ << endl;
        ...
        // cout << PC->valid << " " << IFID->valid << " " << IDEX->valid << " " << EXMO->valid << " " << MOWB->valid << endl << endl;
    }
...
{
    int a = ((RS1.length() > 0) ? stoi(RS1, nullptr, 2) : 0);
    int b = stoi(RS2, nullptr, 2);
    int res = 0;
    switch (AluSelect)
    {
    case 1:
...
string Registers::Read_Reg_Value(string s)
{
    ...
    if ((regIndex != -1) && (Reg_Curr_Writing_Cnt[regIndex] == 0)) return Register_Val[regIndex];
    else
    {
        return "WAIT"; // check
    }
...
void Registers::Print_Vals()
{
    for (int i = 0; i < 32; i++)
    {
        cout << "x" << i << " -> " << this->Register_Val[i] << endl;
    }
    cout << endl;
}
...
{
    Memory_Val.resize(20, "00000000000000000000000000000000");
}
...
{
    for (int i = 0; i < this->Memory_Val.size(); i++)
    {
        cout << "Memory" << i << " -> " << this->Memory_Val[i] << endl;
    }
    cout << endl;
}
...
void Pipeline::Ins_Exe(_IFID *IFID, _IDEX *IDEX, _EXMO *EXMO, _PC *PC)
{
    ...
    if(IDEX->RS1.length() != 0 ) ALUZeroFlag = (IDEX->RS1 == IDEX->RS2);
    EXMO->CW = IDEX->CW;
    EXMO->RS2 = IDEX->RS2;
    EXMO->RD = IDEX->RD;
...
vector<string> Instructions = {
    ...
    "00000010000000001000000101100111",
    "00000000001000001000001001100011",
    "00000000011000001010100000100011",
    "00000001000000001010000110000011",
    "00000000011000011001000100010011",
    "00000000000000101010000100110111"
};
*/