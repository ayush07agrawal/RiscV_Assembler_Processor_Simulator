...
int main()
{
    ...
    {
        if(str.length() == 0) continue; // Changed
        ...
        if (type == "R_Type"){
            file2 << R_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "I_Type"){
            file2 << I_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "Load_Type"){
            file2 << Load_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "B_Type"){
            file2 << B_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "J_Type"){
            file2 << J_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "U_Type"){
            file2 << U_Type_obj.get_Binary(str, func) << endl;
        }
        else if (type == "S_Type"){
            file2 << S_Type_obj.get_Binary(str, func) << endl;
        }
    }
}
...
string R_Type::get_Binary(string s, string func){
    ...
    while(i<s.length() && !(s[i] == ' ' || s[i] == ',')) rd += s[i++];
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    string rs1 = "";
    while(i<s.length() && !(s[i] == ' ' || s[i] == ',')) rs1 += s[i++];
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    string rs2 = "";
    while(i<s.length()) rs2 += s[i++];
...
string I_Type::get_Binary(string s, string func)
{
    ...
    if (func != "slli" && func != "srli" && func != "srai")
    {
        ...
        if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 12);
        else Imm = Utility::to_binary_12(Imm);
    }
    else
    {
        Imm = "0000000";
        if (func == "srai") Imm[1] = '1';
        i = s.length();
        while(i>=0 && s[i] != ' ') i--;
        if((s.length()-i > 2) && s[i+2] == 'x') Imm = Imm + Utility::hex_to_binary(s.substr(i+3), 5);
        else Imm = Imm + Utility::to_binary_5(s.substr(i+1));
    }
    string funct3 = getFunc3(func);
    string OpCode = getOpCode();
...
string Load_Type::get_Binary(string s, string func)
{
    ...
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    string Imm = "";
    while(i<s.length() && s[i] != '(') Imm += s[i++];
    if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 12);
    else Imm = Utility::to_binary_12(Imm);
    i++;
    string rs1 = "";
    while(i<s.length() && !(s[i] == ' ' || s[i] == ',' || s[i] == ')')) rs1 += s[i++];
    rd = Registers::getRegister(rd);
    rs1 = Registers::getRegister(rs1);
...
string B_Type::get_Binary(string s, string func)
{
    ...
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    string rs2 = "";
    while(i<s.length() && !(s[i] == ' ' || s[i] == ',')) rs2 += s[i++];
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    rs1 = Registers::getRegister(rs1);
    rs2 = Registers::getRegister(rs2);
    string Imm = s.substr(i);
    if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 13);
    else Imm = Utility::to_binary_13(Imm);
    string funct3 = getFunc3(func);
    string OpCode = getOpCode();
...
string J_Type::get_Binary(string s, string func)
{
    if (func == "jalr")
    {
        ...
        int immS = i;
        while(s[i]!='(') i++;
        string Imm = s.substr(immS, i-immS);
        if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 12);
        else Imm = Utility::to_binary_12(Imm);
        i++;
        string rs1 = "";
    ...
    else
    {
        ...
        string Imm = s.substr(i);
        if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 21); // check
        else Imm = Utility::to_binary_21(s.substr(i));
...
string U_Type::get_Binary(string s, string func)
{
    ...
    while(i<s.length() && (s[i] == ' ' || s[i] == ',')) i++;
    string Imm = s.substr(i);
    if(Imm.length() > 1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 20);
    else Imm = Utility::to_binary_20(Imm);
    rd = Registers::getRegister(rd);
    string OpCode = getOpCode();
