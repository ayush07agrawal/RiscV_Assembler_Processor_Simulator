...
string R_Type::getFunc3(string func){
    if(Func3.find(func) != Func3.end()) return Func3[func];
    else{
...
string R_Type::getFunc7(string func){
    if(Func7.find(func) != Func7.end()) return Func7[func];
    else{
...
string I_Type::get_Binary(string s, string func){
    ...
    if(func != "slli" && func != "srli" && func != "srai"){
        ...
        while(i>=0 && s[i] != ' ') i--;
        Imm = s.substr(i+1, immE-i);
        if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 5);
        else Imm = Utility::to_binary_5(Imm);
        Imm = "0000000" + Imm;
...
string Load_Type::get_Binary(string s, string func){
    ...
    if(Imm.length() > 1 && s[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 12);
    else Imm = Utility::to_binary_12(Imm);
    i++;
    string rs1 = "";
...
string J_Type::get_Binary(string s, string func){
    if(func == "jalr"){
        ...
        rd = Registers::getRegister(rd);
        string Imm = s.substr(i);
        if(Imm.length()>1 && Imm[1] == 'x') Imm = Utility::hex_to_binary(Imm.substr(2), 21);
        else Imm = Utility::to_binary_21(Imm);
        string OpCode = getOpCode();
